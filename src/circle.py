...
changes = (height // 4) * np.sin(2 * np.pi * freq * timepoints) + height // 4
...
while not stop:
    ...
    pygame.draw.circle(background, white, (width // 2, height // 2),
                       changes[i])
    pygame.draw.circle(background, blue, (width // 2, height // 2), new_v)
